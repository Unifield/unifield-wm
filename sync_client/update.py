    ...
    def execute_update(self, cr, uid, ids=None, priorities=None, context=None):
        ...
        def group_import_update_execution(obj, updates):
            ...
            for update in updates:
                ...
                conflict = self._conflict(cr, uid, update.sdref, update.version, context=context)
                if conflict:
                    #2 if conflict => manage conflict according rules : report conflict and how it's solve
                    logs[update.id] = sync_log(self, "Conflict detected! %s" % (conflict or ''), 'warning', data=(update.id, update.fields, update.values)) + "\n"
