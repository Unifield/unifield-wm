...
class msf_budget_tools(osv.osv):
    ...
    def create_budget_lines(self, cr, uid, budget_id, sequence=False, context=None):
        ...
        # UF-2488 get parent budgets (of parent cost center)
        # because we will create missing line of parent budgets
        # (budgets of parent cost centers)
        budget_obj = self.pool.get('msf.budget')
        parent_budget_ids = False
        budget_fields = ['cost_center_id', ]
        # no browse for perf we just want cost center
        # and we not want to load lines
        budget_r = budget_obj.read(cr, uid, [budget_id], budget_fields,
            context=context)
        if budget_r:
            aaa_obj = self.pool.get('account.analytic.account')
            budget_r = budget_r[0]
            parent_cc_ids = aaa_obj._get_parent_of(cr, uid,
                budget_r['cost_center_id'][0], context=context)
            if parent_cc_ids:
                domain = [
                    ('cost_center_id', 'in', parent_cc_ids),
                ]
                parent_budget_ids = budget_obj.search(cr, uid, domain,
                    context=context)
                if parent_budget_ids and budget_id in parent_budget_ids:
                    parent_budget_ids.remove(budget_id)
        ...
        for budget_line in template:
            ...
            if line_type == 'normal':
                ...
                for destination_id in budget_line.get('destination_ids', []):
                    ...
                    budget_line_id = budget_line_obj.create(cr, uid,
                        budget_line_vals, context=context)
                    self._create_missing_parent_budgets_destination_line(cr,
                        uid, parent_budget_ids, budget_line_id,
                        budget_line_vals, context=context)  # UF-2488
    ...
    def _create_missing_parent_budgets_destination_line(self, cr, uid,
        parent_budget_ids, origin_budget_line_id, budget_line_vals,
        context=None):
        # UF-2488
        ...
        if not parent_budget_ids or not budget_line_vals:
            return
        budget_line_obj = self.pool.get('msf.budget.line')
        origin_parent_id = budget_line_vals.get('parent_id', False)
        default_vals = {}
        for m in range(1, 13):
            default_vals['month%d' % (m, )] = 0.  # default month amounts
        ...
        for pb_id in parent_budget_ids:
            ...
            if not parent_line_count:
                ...
                id = budget_line_obj.copy(cr, uid, origin_budget_line_id,
