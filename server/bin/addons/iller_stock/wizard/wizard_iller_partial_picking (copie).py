...
def qty_change(self, cr, uid, field_name, qty, context={}):
    print field_name, qty
...
def _get_moves(self, cr, uid, data, context):
    ...
    for m in pick.move_lines:
        ...
        _moves_fields['move%s' % m.id] = {
                ...
                'type' : 'float', 'required' : True, 'default' : make_default(quantity), 'on_change': "qty_change(cr, uid, field_name, quantity, context=context)"}
        ...
        _moves_arch_lst.append('<field name="difference%s" readonly="0"/>' % (m.id))
        _moves_fields['difference%s' % m.id] = {
            'string': 'Diff.',
            'type': 'float',
            'required': False,
            'default': '0.00' }

        _moves_arch_lst.append('<field name="reliquat%s" nolabel="1"/>' % (m.id))
        _moves_fields['reliquat%s' % m.id] = {
            'type': 'boolean',
            'required': False,
            'default': False,
