...
class wizard_export_tarif_hilton(osv.osv_memory):
    ...
    def build_product_line(self, cr, uid, product, date_debut, date_fin, prix):
        export = "U;"
        ...
        for item_id in item_ids:
            ...
            if item.product_id:
                ...
                if not prix.get(item.product_id.id) :
                    ...
                    fichier_tmp += self.build_product_line(cr, uid, item.product_id, date_debut, date_fin, price)
            else:
                if item.categ_id:
                    ...
                    for product_id in product_ids:
                        if not prix.get(product_id):
                            pro = product_obj.browse(cr, uid, product_id)
                            price = pricelist_obj.price_get(cr, uid, [pricelist_id],
                            ...
                            fichier_tmp += self.build_product_line(cr, uid, pro, date_debut, date_fin, price)
                            #~ inthread = threading.Thread(target=self.get_price_for_product, 
                                #~ args=(cr.dbname, uid, fichier_tmp, product_id, prix, pricelist_id, this.from_date, date_debut, date_fin, qty, partner_id))
                            #~ inthread.start()
                            #~ list_threads.append(inthread)
                else:
