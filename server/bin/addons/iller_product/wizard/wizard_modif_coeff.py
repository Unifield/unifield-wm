...
class wizard_choose_category(osv.osv_memory):
    ...
    def _choose(self, cr, uid, ids, context={}):
        ...
        for cat in self.browse(cr, uid, ids):
            context.update({'category_id': cat.category_id.id, 'wizard': 1})
            ...
            coeffs.sort()
            for c in coeffs:
                coeff_obj.create(cr, uid, {'name': c, 'coeff_new': c, 'category_id': cat.category_id.id}, context=context)
