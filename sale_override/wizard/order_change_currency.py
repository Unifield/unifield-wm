...
class sale_order_change_currency(osv.osv_memory):
    ...
    def compute_pricelist_rate(self, cr, uid,
                               from_pricelist,
                               to_pricelist, context=None):
        '''
        ...
        if not from_pricelist and not to_pricelist:
            raise osv.except_osv(
                _('Error'),
                _('Please give two pricelists')
            )

        pricelist_obj = self.pool.get('product.pricelist')
        cur_obj = self.pool.get('res.currency')

        from_currency = pricelist_obj.browse(
            cr, uid, from_pricelist).currency_id.id
        to_currency = pricelist_obj.browse(
            cr, uid, to_pricelist).currency_id.id

        rate = cur_obj.compute(cr, uid, from_currency, to_currency, 1.00)

        return rate

    def currency_change(self, cr, uid, ids,
                        old_pricelist_id,
                        new_pricelist_id):
        '''
        ...
        if old_pricelist_id and new_pricelist_id:
            rate = self.compute_pricelist_rate(
                cr, uid, old_pricelist_id, new_pricelist_id)
        ...
        for wiz in self.browse(cr, uid, ids, context=context):
            for line in wiz.order_id.order_line:
                new_price = currency_obj.compute(
                    cr,
                    uid,
                    wiz.old_pricelist_id.currency_id.id,
                    wiz.new_pricelist_id.currency_id.id,
                    line.price_unit,
                    round=False,
                    context=context
                )
            ...
            order_obj.write(cr, uid, wiz.order_id.id, {
                'pricelist_id': wiz.new_pricelist_id.id,
            }, context=context)

            msg = _('The currency of the FO \'%s\' has been changed. '\
                    '(old currency: %s - new currency: %s)') % (
                        wiz.order_id.name,
