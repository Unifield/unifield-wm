...
class stock_move(osv.osv):
    ...
    def _check_tracking(self, cr, uid, ids, context=None):
        ...
        """
        for move in self.browse(cr, uid, ids, context=context):
            no_traceability = move.location_id.no_traceability or (move.location_dest_id.no_traceability and move.location_id.usage == 'inventory')
            if move.state == 'done' and move.location_id.id != move.location_dest_id.id:
                if move.product_id.batch_management and not no_traceability:
                ...
                if move.product_id.perishable and not no_traceability:
                    if not move.prodlot_id and move.product_qty:
            ...
            if not move.prodlot_id and move.product_qty and \
               (not no_traceability and move.state == 'done' and \
               ( \
...
class stock_inventory_line(osv.osv):
    ...
    def common_on_change(self, cr, uid, ids, location_id, product, prod_lot_id, uom=False, to_date=False, result=None):
        ...
        if location_id:
            ...
            if self.pool.get('stock.location').browse(cr, uid, location_id).no_traceability:
                result.setdefault('value', {}).update({'hidden_perishable_mandatory': False, 
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if vals.get('location_id', False):
            # complete hidden flags - needed if not created from GUI
            location = loc_obj.browse(cr, uid, vals.get('location_id'), context=context)
            if location.no_traceability:
                vals.update(hidden_batch_management_mandatory=False,
                            hidden_perishable_mandatory=False,
                            prod_lot_id=False,
                            expiry_date=False)
    ...
    def _get_checks_all(self, cr, uid, ids, name, arg, context=None):
        ...
        for obj in self.browse(cr, uid, ids, context=context):
            ...
            if obj.product_id.narcotic:
                result[obj.id]['np_check'] = True
            # lot management
            if obj.product_id.batch_management and not obj.location_id.no_traceability:
                result[obj.id]['lot_check'] = True
            # expiry date management
            if obj.product_id.perishable and not obj.location_id.no_traceability:
                result[obj.id]['exp_check'] = True
    ...
    def _check_batch_management(self, cr, uid, ids, context=None):
        ...
        '''
        for obj in self.browse(cr, uid, ids, context=context):
            if obj.inventory_id.state not in ('draft', 'cancel') and obj.product_id.batch_management and not obj.location_id.no_traceability:
    ...
    def _check_perishable(self, cr, uid, ids, context=None):
        ...
        """
        for obj in self.browse(cr, uid, ids, context=context):
            if obj.inventory_id.state not in ('draft', 'cancel') and obj.product_id.perishable and not obj.product_id.batch_management and not obj.location_id.no_traceability:
    ...
    def _check_prodlot_need(self, cr, uid, ids, context=None):
        ...
        for obj in self.browse(cr, uid, ids, context=context):
            if obj.prod_lot_id:
                if (not obj.product_id.perishable and not obj.product_id.batch_management) or obj.location_id.no_traceability:
                    return False
        return True
